...
Resource* Font::GetFont( File* file, Kvp kv ) {
	...
	*/
	/*
	int cnt;
	char ** fonts = XListFonts(xcbBackend::display, "*", 10, &cnt);
	for(int i=0; i < cnt; i++) {
	...
	// ::Font fnt = XLoadFont( xcbBackend::display, font->fontname.c_str() );
	FontPriv* priv = (FontPriv*)font->priv;
	// priv->font = fnt;
	// priv->font_struct = ::XQueryFont(xcbBackend::display, DefaultGC(xcbBackend::display, 0));
	priv->font_struct = ::XLoadQueryFont(xcbBackend::display, font->fontname.c_str());
	if(!priv->font_struct) {
